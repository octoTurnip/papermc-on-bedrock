...
cd /minecraft || {
	echo "ERROR: Failed to change directory to /minecraft. Exiting."
	exit 1
}
...
latestPaperBuild=$(curl -s --no-progress-meter "https://api.papermc.io/v2/projects/paper/versions/${PaperVersion}/builds" | jq -r '.builds | map(select(.channel == "STABLE") | .build) | .[-1]')
...
if [[ "$experimentalBuilds" == "on" ]]; then
	latestPaperBuild=$(curl -s --no-progress-meter "https://api.papermc.io/v2/projects/paper/versions/${PaperVersion}/builds" | jq -r '.builds | map(select(.channel == "BETA") | .build) | .[-1]')
	latestPaperBuild="$latestPaperBuild" yq -i '.paperUpdate.latestPaperBuild = env(latestPaperBuild)' logs/startupVars.yaml
fi
